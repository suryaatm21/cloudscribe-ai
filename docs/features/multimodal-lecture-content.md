...
- **`text` empty** — visual-only. Two transcription outcomes reach this shape; they share the assembly route but **must not** share a user-facing status:
  - **`no_audio_detected`** — no speech existed. Slide-derived notes are **complete**. Nothing is missing. Two distinct routes converge here (same terminal status, same assembler meaning):
    - **No audio stream** — the uploaded file has no audio track at all (screen recording with mic off, synthetic test pattern). Detected during FLAC extraction via ffprobe before Speech runs.
    - **Audio present, no speech** — Speech returns a well-formed result with zero usable segments (silence, room noise, music without lyrics). Detected in the transcript parser after Speech completes.
    Both mean a **complete** visual-only lecture, not a degraded one. If the assembler later needs to distinguish the routes (for example to skip a billed Speech call retrospectively), that is a metadata question — not a second status.
...
**Worked example (silent slides, no audio track).** A student uploads a 40-minute screen recording of a slide deck with the microphone off — the file contains video but **no audio stream**. ffprobe during extraction detects the missing track; the transcript document becomes `no_audio_detected` without a Speech job or signed transcript URL. The assembler does not stop. It waits for scene detection, the sample floor, hash dedup, OCR, and descriptions. If those produce usable slide text and diagrams, it emits lecture-content with empty `text` and populated `visuals`. Notes run on OCR + descriptions. The outcome is **visual-only, complete**. The UI must not say the lecture failed, and must not imply missing audio content.

**Worked example (silent slides, audio track present).** Same lecture, but the container includes a silent audio track (some recorders always mux audio). Speech returns a well-formed empty result. The transcript document becomes `no_audio_detected` via the parser path; the watch page still has no transcript URL to sign. Assembly and UX are identical to the no-stream case above — **visual-only, complete**.
