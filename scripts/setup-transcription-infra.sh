...
Usage: ./scripts/setup-transcription-infra.sh \
  ...
  --push-identity <SERVICE_ACCOUNT_EMAIL> \
...
Creates the infrastructure required for Sprint 2 transcription:
  ...
  7. Prefix-filtered GCS notification on raw/
...
SERVICE_ACCOUNT="${SERVICE_ACCOUNT:-$PUSH_IDENTITY}"
...
function preflight_act_as() {
  ...
  if ! gcloud iam service-accounts describe "$PUSH_IDENTITY" \
    --project "$PROJECT_ID" >/dev/null 2>&1; then
    echo "ERROR: cannot describe ${PUSH_IDENTITY}."
    echo "The creator needs iam.serviceAccounts.actAs on this account"
    echo "to configure OIDC push subscriptions."
    exit 1
  fi
}
...
TRANSCRIPTS_LIFECYCLE="$(cat <<EOF
{
  "rule": [
    {
      "action": {"type": "Delete"},
      "condition": {"age": ${LIFECYCLE_DAYS}, "matchesPrefix": ["raw/"]}
    }
  ]
}
EOF
...
if echo "$EXISTING_NOTIFICATION" | grep -q "${READY_TOPIC}" && \
   echo "$EXISTING_NOTIFICATION" | grep -q "raw/"; then
  echo "GCS notification on raw/ -> ${READY_TOPIC} already exists."
else
